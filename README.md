...
Épée is an open, AI-grade ASL corpus: **600 clips, 150 phrases, 4 native Deaf signers**, released as
MediaPipe keypoints with expert gloss segmentation. It is **fully parallel** — every phrase is signed
by all four signers — which makes it one of the few public sign language datasets where signer
variation can be isolated from phrase variation.
...
```python
clips = epee.parallel_group(7)   # phrase #7, signed by all four signers
[(c.signer_id, round(c.duration_s, 2)) for c in clips]
# [('ALPHA', 7.1), ('BRAVO', 3.47), ('CHARLIE', 3.42), ('DELTA', 3.97)]
```
...
Phrase #7, *“How old are you?”*, signed by four native Deaf signers:
...
Same phrase, same language, same recording protocol. A **2.1× spread in duration**, a **2.7× spread in
...
Chance is 4.2%. A signer the model *has* seen scores 65%, so four signers recovers most of the gap to
the ceiling. The curve has not flattened, in signers or in data.

**Sign language AI is data-bound, not model-bound.** A bigger model does not close the
one-signer-to-four-signer gap, because it is a property of the data.
...
| `parallel_group(n)` | phrase *n* as signed by all four signers |
...
@misc{clerc_epee_2026,
  title  = {CLERC Épée v0.2: A Multi-Signer ASL Keypoint Corpus},
